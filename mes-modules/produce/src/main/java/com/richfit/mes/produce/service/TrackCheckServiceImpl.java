...
public class TrackCheckServiceImpl extends ServiceImpl<TrackCheckMapper, TrackCheck> implements TrackCheckService {
    ...
    public CommonResult<IPage<TrackItem>> queryQualityPage(int page, int limit, String branchCode, String isExistQualityCheck, String isScheduleComplete, String startTime, String endTime, String trackNo, String productNo, String tenantId, Boolean isRecheck, String drawingNo, String order, String orderCol) {
        try {
            ...
            queryWrapper.and(wrapper -> wrapper.and(w -> w.eq("is_operation_complete", 1).and(w2 -> w2.eq("quality_check_by", SecurityUtils.getCurrentUser().getUsername()).or(wrapper1 -> wrapper1.eq("quality_check_by", "/").eq("branch_code", branchCode)))).or().eq("opt_type", 5));
            //是否质检
