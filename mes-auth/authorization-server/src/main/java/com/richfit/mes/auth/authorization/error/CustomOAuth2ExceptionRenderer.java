package com.richfit.mes.auth.authorization.error;
...
public class CustomOAuth2ExceptionRenderer {
    ...
    public void handleResponseBeanResponse(ResponseEntity<?> responseEntity, ServletWebRequest webRequest)
        ...
        if (body != null) {
            // CommonResult
            CommonResult<?> commonResult = new CommonResult(responseEntity.getStatusCode().value(),"",body);
            writeWithMessageConverters(commonResult, inputMessage, outputMessage);
        } else {
            // flush headers
