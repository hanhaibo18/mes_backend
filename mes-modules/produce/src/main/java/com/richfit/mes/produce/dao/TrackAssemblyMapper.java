...
public interface TrackAssemblyMapper extends BaseMapper<TrackAssembly> {
    ...
    @Select("SELECT\n" +
            ...
            "\tpta.unit,\n" +
            "\tmrd.quantity,\n" +
            "\tmrd.order_quantity AS orderQuantity \n" +
            "FROM\n" +
            "\tproduce_track_assembly AS pta\n" +
            "\tLEFT JOIN ( SELECT material_num, sum( quantity ) AS quantity, sum( order_quantity ) AS order_quantity FROM produce_material_receive_detail WHERE state = '1' GROUP BY material_num ) mrd ON pta.material_no = mrd.material_num \n" +
            "WHERE\n" +
