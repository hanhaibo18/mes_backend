...
import com.richfit.mes.base.service.PdmMesBomService;
...
@Api("图纸")
@RestController
@RequestMapping("/api/base/mes/pdmBom")
public class PdmMesBomController {
    ...
    public CommonResult<List<PdmMesBom>> getChildBomList(String id, String ver) {
        String opId = id + "@" + ver;
        QueryWrapper<PdmMesBom> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("p_id", opId).orderByAsc("order_no+1");
        return CommonResult.success(pdmMesBomService.list(queryWrapper));
    }
    ...
    public void exportBom(String drawingNo, String ver, String dataGroup, HttpServletResponse rsp) {
        try {
            ...
            QueryWrapper<PdmMesBom> query = new QueryWrapper<>();
